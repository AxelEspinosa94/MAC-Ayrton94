...
> Interest level -> ordinal quantitative
> Income bracket -> ordinal qualitative
> age -> ratio quantitative
...
**a)** Compute the **mean**, **median**, and **mode**.
   ...
   -  $m(X)=m\{45, 52, 52, 60, 61, 68, 70, 70, 70, 92\} = \frac{61+68}{2} = \frac{129}{2} = 64.5$
   -  $M(X) = 70$
...
> Mean is severely affected by outliers, one example of this can be seen in **b)** if policymaker relies on the mean, he could conclude population is wealthier than it in reality is.
...
> This is due to the median definition. Outliers lay at the extreme values and they are typically a minority of all data.
...
**a)** Compute the **sample variance** and **sample standard deviation**.
   ...
   -  $s^2 =\frac{1}{9}\sum_{i=1}^10 (x_i -\bar{x})^2 ~ 175.78$
   -  $s=\sqrt{175.78} ~ 13.26$

**b)** Compute the **coefficient of variation** (CV = $s / \bar{x}$).

$$
CV = \frac{s}{\bar{x}} = \frac{13.26}{64}=0.2072
$$
...
> Clear upward trend; sales increasing over time.
